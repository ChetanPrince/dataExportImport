...
function filterExpenses() {
    ...
    for (const row of tableBody.rows) {
        ...
        if (filterValue === "" || filterValue === rowCategory) {
            if (!isNaN(value)) {
                total += value;
            }
    ...
    const filteredText = filterValue ? `Total ${filterValue} Expenses: $${total.toFixed(2)}` : "Total Expenses: $0.00";
    document.getElementById("filteredExpenses").textContent = filteredText;
}
